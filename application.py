...
@app.route('/question_atop')#「診断：仕事・作業編」
def question_atop():
    return render_template('quiz_a/cover_a.html')
@app.route('/question_btop')#「診断：仕事・作業編」
def question_btop():
    return render_template('quiz_b/cover_b.html')

@app.route('/nigate_a1')# 「準備中」
def nigate_a1():
    return render_template('nigate_a1.html')
@app.route('/often-forget-items')#「置き忘れ」
def nigate_a2():
    return render_template('nigate_a2.html')
@app.route('/always-late')#「遅刻」
def nigate_a3():
    return render_template('nigate_a3.html')
@app.route('/awkward-in-conversations')#「雑談」
def nigate_a4():
    return render_template('nigate_a4.html')
@app.route('/forget-todo')#「やるべきことを忘れる」
def nigate_a5():
    return render_template('nigate_a5.html')
@app.route('/slow-at-routine-task')#「単純作業を素早くこなす」
def nigate_a6():
    return render_template('nigate_a6.html')
@app.route('/anxiety')#「不安と緊張」
def nigate_a7():
    return render_template('nigate_a7.html')
@app.route('/relax')#「リラックス法」
def nigate_relax():
    return render_template('nigate_relax.html')
@app.route('/nigate_z1')#調整用ページ（利用者には表示されない）
def nigate_z1():
    return render_template('nigate_z1.html')
@app.route('/nigate-list')#苦手対策一覧
def nigate_list():
    return render_template('nigate_list.html')
